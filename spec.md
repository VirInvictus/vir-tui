...
3. **Architecture**:
   - `core.py`: ANSI state logic, color formats, `tqdm` handling (the real tqdm is re-exported when installed, a minimal fallback otherwise).
   - `menu.py`: everything interactive:
     - the curses arrow-key menu (`tui_select`, sections/aliases/letter keys, auto-generated typed-input fallback via `build_fallback`; aliases and letter keys steer the text fallback only, while the curses menu navigates by arrows, mouse, and the filter),
   ...
   - Public style constants (`CP_FRAME` … `CP_HINT`) let hosts render their own widgets into the session screen without mirroring private ids. Ids 1-6 are re-initialized at every screen open and are reserved: hosts use the constants as-is or pick ids 7+ for private pairs.
