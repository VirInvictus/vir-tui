...
- **`tui_page(title, content)`** — scrollable, pannable pager for long text: arrows scroll, `←→` pan (8 columns), `/` search with `n`/`N` match jumping (wrap-around), `g`/`G` top/bottom, mouse wheel pages, `q`/Esc/Enter closes. Tab stops expand; `\r` and `\x00` frames are stripped. Without curses: prints and pauses.
